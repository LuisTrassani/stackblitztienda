...
export default function Casacon() {
  ...
  const agregarAlCarrito = (producto: Producto) => {
    ...
    if (!talle) { alert("Selecciona un talle primero"); return; }
    const existeIndice = carrito.findIndex(item => item.id === producto.id && item.talleSeleccionado === talle);
    if (existeIndice !== -1) {
      const nuevo = [...carrito];
      nuevo[existeIndice].cantidad += 1;
      setCarrito(nuevo);
    } else {
      setCarrito([...carrito, { ...producto, talleSeleccionado: talle, cantidad: 1 }]);
    }
  };
  ...
  if (cargando) return <div className="min-h-screen flex items-center justify-center font-black uppercase italic">Buscando Casacas...</div>;
        ...
        <div className="flex animate-marquee whitespace-nowrap">
          {[1,2,3,4].map(i => (
            <span key={i} className="text-xl font-black uppercase italic mx-10">
              🔥 ENVÍOS A TODO EL PAÍS - VENTAS POR MAYOR Y MENOR - CASACÓN OFICIAL 🔥
            </span>
          ))}
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {productosFiltrados.map((prod) => (
            <div key={prod.id} className="bg-white rounded-3xl border border-gray-100 p-4 shadow-sm flex flex-col group">
              <div className="aspect-square bg-gray-50 rounded-2xl mb-4 flex items-center justify-center overflow-hidden">
                <img src={imagenSeleccionada[prod.id] || prod.imagen_url || prod.imagen || prod.img} className="w-full h-full object-contain group-hover:scale-110 transition-all p-2" />
              </div>
              
              {/* Mini Galería */}
              <div className="flex gap-1 justify-center mb-4">
                {[prod.imagen_url || prod.imagen || prod.img, prod.imagen_url_2, prod.imagen_url_3].filter(Boolean).map((img, i) => (
                  <button key={i} onClick={() => setImagenSeleccionada({...imagenSeleccionada, [prod.id]: img!})} className="w-8 h-8 border rounded overflow-hidden bg-white">
                    <img src={img} className="w-full h-full object-contain" />
                  </button>
                ))}
              </div>

              <h3 className="text-xs font-bold uppercase text-gray-500 mb-1">{prod.nombre}</h3>
    ...
    <div className="bg-white rounded-[2.5rem] border-4 border-black p-8 shadow-[15px_15px_0px_0px_rgba(0,0,0,1)] relative">
        ...
        {carrito.map((item, index) => (
          <div key={index} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-xs uppercase leading-tight">{item.nombre} ({item.talleSeleccionado})</p>
                <p className="text-[10px] font-black text-pink-600">${(item.precio * item.cantidad).toLocaleString('es-AR')}</p>
              </div>
            </div>

            {/* CONTROL DE CANTIDAD */}
            <div className="flex items-center gap-2 bg-white border-2 border-black rounded-lg px-2 py-1">
              <button onClick={() => cambiarCantidad(item.id, item.talleSeleccionado, -1)} className="font-black text-xs px-1 hover:text-pink-600">-</button>
              <span className="font-black text-xs w-4 text-center">{item.cantidad}</span>
              <button onClick={() => cambiarCantidad(item.id, item.talleSeleccionado, 1)} className="font-black text-xs px-1 hover:text-green-600">+</button>
            </div>
     ...
     <section className="bg-black text-white py-20 px-4 mt-20 border-t-8 border-yellow-400">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-16 text-left">
          ...
          {/* Botones de Redes */}
          <div>
            <h4 className="font-black italic text-3xl mb-6 uppercase tracking-tighter text-white">Nuestras Redes</h4>
            <div className="flex gap-6">
              {/* Instagram */}
              <a href="https://www.instagram.com/casacon.ch/" target="_blank" className="bg-white text-black w-12 h-12 rounded-full flex items-center justify-center hover:bg-pink-600 hover:text-white transition-all transform hover:-rotate-12">
              ...
              {/* WhatsApp */}
              <a href="https://wa.me/543731652931" target="_blank" className="bg-white text-black w-12 h-12 rounded-full flex items-center justify-center hover:bg-green-500 hover:text-white transition-all transform hover:rotate-12">
